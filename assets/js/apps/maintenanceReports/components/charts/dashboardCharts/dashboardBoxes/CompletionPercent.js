...
function CompletionPercent({properties, calculatePercentageChange, InfoBox}) {
  ...
  useEffect(() => {
    if (properties && properties.length) {
      const fetchData = async () => {
        setFetching(true);
        const result = await axios(`/api/maintenance_reports?dates&properties=${properties}&type=info_box_completion_percent`);
        setData(result.data);
        setFetching(false);
      }
      fetchData();
