...
import {Button, Table, Alert, Card, CardHeader, CardBody} from 'reactstrap';
...
class Day extends React.Component {
  ...
  render() {
    const {date, showings, openings, closures} = this.props;
    const {error} = this.state;
    return <Card>
      <CardHeader className="d-flex justify-content-between align-item-center mb-2">
        ...
        <Table bordered>
          ...
          {openings.map(o => {
            ...
            return [...new Array(blocks)].map((b, index) => {
              ...
              const closuresSlots = closures.filter(c => c.start_time <= start && c.end_time >= start + interval);
              return <tr key={index}>
