...
function completedInfo(order) {
  if (!order.assignments) return
  const a = order.assignments[0];
	if (a) return <span>Completed by <b>{a.tech}</b> on <b>{moment.utc(a.completed_at).local().format("MM/DD/YY h:mmA")}</b></span>
	return ""
}

function ratedInfo(order) {
  if (!order.assignments) return
  const a = order.assignments[0];
  if (a) return <Rate disabled defaultValue={a.rating} />
  return ""
}

function commentInfo(order) {
  if (!order.assignments) return
  const a = order.assignments[0];
  if (a) return a.resident_comment
  return ""
}

function completionTime(order) {
  if (!order.assignments) return
  const a = order.assignments[0];
  if (a) return moment(a.completed_at).utc().to(moment(a.inserted_at).utc(), true)
}
...
const CompletedDescription = ({order, action, setAction, setChanged, edit}) => {
	return <>
		...
		<Col sm={{span: 24}} md={{span: 12}}>
          ...
          <Item span={4}>{completedInfo(order)}</Item>
          <Item span={4} label={<span style={styles.label}>Completion Time</span>}>{completionTime(order)}</Item>
          <Item span={4} label={<span style={styles.label}>Rating</span>}>{ratedInfo(order)}</Item>
          <Item span={4} label={<span style={styles.label}>Resident Comment</span>}>{commentInfo(order)}</Item>
					{callbackItem(order)}
          <Item span={4} />
