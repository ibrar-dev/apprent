...
config :app_count, :queue, AppCount.Support.FakeQueue
config :app_count, :state_cache, AppCount.Support.FakePersist
...
# In test env, DomainEventServer does not listen for any topic's DomainEvents
config :app_count, :topics_to_store, []
...
config :app_count, :boundary, %{payment: AppCount.Core.FakePaymentBoundary}

config :app_count, :generate_pin_fn, fn -> "PIN-FOR-TESTING" end
...
config :app_count,
  # ex: AppCount.adapters(:bank_account, PayscapeAdapter)
  adapters: [
    ...
    credit_card: AppCount.Support.Adapters.CreditCardAuthorizeFake,
    bank_account: AppCount.Support.Adapters.BankAccountPayscapeFake,
    sleeper: AppCount.Support.Adapters.NoSleep
  ]
