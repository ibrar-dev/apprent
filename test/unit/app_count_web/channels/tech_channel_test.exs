defmodule AppCountWeb.TechChannelTest do
  ...
  test "NOTIFICATION_TOKEN", %{socket: socket} do
    params = %{"name" => "Joke"}
  ...
  test "send_tech_notification", %{tech: tech} do
    %AppCount.Core.ClientSchema{name: "dasmen", attrs: tech.id}
    |> TechChannel.send_tech_notification("title A", "body B")

    assert_push "NOTIFICATION", %{title: "title A", body: "body B"}
  end

  test "send_tech_data", %{tech: tech} do
    %AppCount.Core.ClientSchema{name: "dasmen", attrs: tech.id}
    |> TechChannel.send_tech_data()
  ...
  test "send_order_data", %{tech: tech, assignment: assignment} do
    %AppCount.Core.ClientSchema{name: "dasmen", attrs: tech.id}
    |> TechChannel.send_order_data(assignment.order.id)

    assert_push "ORDER_DATA", %{assignments: [%{"id" => _}]}
  end
end
